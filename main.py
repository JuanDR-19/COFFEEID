...
def create_ps(positive_samples, target_size):
    ...
    for sample in positive_samples:
        ...
        resized_sample = cv2.resize(sample, target_size)  # target_size debe ser una tupla (width, height)
        samples.append(resized_sample)
    print("Muestras positivas redimensionadas.")
    return samples

def train_cnn(positive_samples, negative_images_dir, model_filename, input_shape, num_classes, epochs):
    ...
    for image_path in negative_images:
        ...
        if image is not None:
            ...
            # Redimensionar la imagen al tamaño deseado
            resized_image = cv2.resize(image, input_shape[:2])  # target_size debe ser una tupla (width, height)
...
def main():
    ...
    if not os.path.isfile(model_filename):
        ...
        input_shape = (target_size[0], target_size[1], 1)  # Tamaño de entrada de la CNN (agregando una dimensión de canal)
        train_cnn(positive_samples, negative_images_dir, model_filename, input_shape, num_classes, epochs)
